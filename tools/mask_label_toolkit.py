...
class MaskLabelToolkit:
    ...
    def _init_sam_predictor(self, model_type="vit_t"):
        ...
        chkpt = PROJ_ROOT / f"models/sam/sam_{model_type}.pth"
        sam = sam_model_registry[model_type](checkpoint=chkpt).to(self._device)
        sam.eval()
    ...
    def _on_save_mask(self):
        ...
        mask = self.get_mask()
        cv2.imwrite(
            str(self._save_folder / f"{self._image_name.replace('color', 'mask')}.png"),
            mask,
        )
        # Save mask overlay
        vis_image = self._overlay_mask(self._raw_image, mask, alpha=0.632)
        vis_image = cv2.cvtColor(vis_image, cv2.COLOR_RGB2BGR)
        cv2.imwrite(
            str(self._save_folder / f"{self._image_name.replace('color', 'vis')}.jpg"),
            vis_image,
        )
    ...
    def _on_filedlg_done(self, path: str):
        ...
        self._save_folder = path.parent.parent / "segmentation/init_segmentation"
    ...
    def _overlay_mask(
        ...
        for label in unique_labels:
            mask_color = SEG_CLASS_COLORS[label % len(SEG_CLASS_COLORS)]
